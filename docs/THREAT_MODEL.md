...
- desconexão, timeout ou energia insuficiente durante uma operação;
- backend USB reutilizado após encerramento ou sem lifecycle explícito;
- dependência comprometida ou segredo no CI.

## Controles

Streaming, limites de tamanho, magic bytes, hashes, confirmação textual, alvo único, allowlist, feature flags, timeouts, cancelamento cooperativo, lifecycle explícito dos backends, logs redigidos, HTTPS, atualizações de dados assinadas, CodeQL, revisão bloqueante de dependências, scanner de segredos, SBOM para futuras releases, revisão CodeRabbit e fuzzing.
